...
combine_chains <- function(data_dir, output_dir = data_dir,
  ...
  file_info <- data.frame(file = all_files,
                          ...
                          chain = as.numeric(gsub('.rdata', '', sapply(split , function(x) tail(x,1)))),
  ...
  for(sp in species){
    ...
    chain_files <- species_info$file[species_info$final_iteration == max(species_info$final_iteration)]
    ...
    sims.list <- lapply(unique(species_info$chain), FUN = function(chain){
      ...
      # Load the last chain
      last_chain <- chainfiles$file[chainfiles$final_iteration == max(chainfiles$final_iteration)]
      load(last_chain)
      sims.matrix <- out_integrated_model$BUGSoutput$sims.matrix
      ...
      if(!is.null(target)){
        if(nrow(sims.matrix) >= target){
          ...
          if(verbose) cat(paste0('trimmed to it-', nrow(sims.matrix), ' '))
        } else {
          ...
          while(nrow(sims.matrix) < target){
            ...
            load(chainfiles$file[chainfiles$final_iteration == daisies[d]])
            d <- d + 1
