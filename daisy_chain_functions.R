  ...
  for(i in 1:nDaisy){
    if(model == "integrated"){
      ...
      chain_out <- daisy_not_integrated(rdataFile = rdataFile, outDir = outDir, 
                                        ...
                                        quiet = TRUE, update = update)
      #    tot <- chain_out$tot.it
      rdataFile <- chain_out$out_HRS_model
      
      
      # return(chain_out$out_HRS_model)
...
daisy_not_integrated <- function(rdataFile, outDir = getwd(),
  ...
  if(is.character(rdataFile)){
    ...
    out_HRS_model <- rdataFile
    if('filename' %in% names(out_HRS_model)){
      rdataFile <- out_HRS_model$filename  
    } else { # make up a name
      rdataFile <- paste0(paste(out_HRS_model$SPP_NAME,
                                gsub('.txt$', '', tolower(basename(out_HRS_model$model.file))),
                                sep = '_'),
                          'HRS.rdata')
      rdataFile <- file.path(outDir, rdataFile)
    }
  ...
  cat('\nLoaded', out_HRS_model$n.iter, 'iterations... ', 
      file = paste0(outDir, "progress.txt"), append = TRUE)
  
  # if((out_HRS_model$n.iter + n.iter) > total.it) n.iter <- total.it - out_HRS_model$n.iter
  ...
  if(update == TRUE){
    if(!obj){
      if(quiet){
        cat('compiling... ')
        invisible(capture.output(R2jags:::recompile.rjags(out_HRS_model, progress.bar = 'none')))
      } else {
        R2jags:::recompile.rjags(out_HRS_model)
        cat('\nRecompiled', file = paste0(outDir, "progress.txt"), append = TRUE)
    ...
    new_out_HRS_model <- R2jags:::update.rjags(out_HRS_model,
                                               n.iter = n.iter,
                                               n.thin = n.thin)
    
    cat('\nUpdated', file = paste0(outDir, "progress.txt"), append = TRUE)
    
    new_out_HRS_model$SPP_NAME <- out_HRS_model$SPP_NAME
    new_out_HRS_model$min_year <- out_HRS_model$min_year
    new_out_HRS_model$max_year <- out_HRS_model$max_year
    out_HRS_model <- new_out_HRS_model
    
  }
  
  
  cat('output', out_HRS_model$n.iter, '\n', 
      file = paste0(outDir, "progress.txt"), append = TRUE)
  
  return(list(out_HRS_model = out_HRS_model))
}
